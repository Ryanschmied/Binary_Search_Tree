...
bool Search_tree<Type>::insert( Type const &obj ) {
	if ( empty() ) {
        ...
        //Insert root node into Linked List
...
bool Search_tree<Type>::Node::insert( Type const &obj, Search_tree<Type>::Node *&to_this ) {
	if ( obj < node_value ) {
		if ( left_tree == nullptr ) {
            ...
            //Insert smaller node into linked list
			...
			if ( left_tree->insert( obj, left_tree ) ) {
				update_height();
                //Check if rotate is required to balance tree
                rotate(to_this);
				return true;
			} else {
				return false;
		...
		if ( right_tree == nullptr ) {
            ...
            //Insert larger node into linked list
...
bool Search_tree<Type>::Node::erase( Type const &obj, Search_tree<Type>::Node *&to_this ) {
	if ( obj < node_value ) {
        ...
        //Manage node connections in linked list to remove to_this from the linked list
...
template <typename Type>
bool Search_tree<Type>::Node::rotate( Search_tree<Type>::Node *&to_this )
{
    //Right imbalance
    if(to_this->right_tree->height() - to_this->left_tree->height() >= 2)
    {
        if(to_this->right_tree->right_tree->height() > to_this->right_tree->left_tree->height()) //RR rotation
        {
            Node *right = to_this->right_tree;
            Node *temp = right->left_tree;
            right->left_tree = to_this;
            to_this->right_tree = temp;
            to_this = right;
        ...
        {
            Node *b  = right_tree, *d  = b->left_tree, *DL = d->right_tree, *DR = d->left_tree;
            d->right_tree = b;
            d->left_tree = to_this;
            to_this = d;
            b->left_tree = DL;
    ...
    //Left imbalance
    else if(to_this->left_tree->height() - to_this->right_tree->height() >= 2)
    {
        if(to_this->left_tree->left_tree->height() > to_this->left_tree->right_tree->height()) //LL rotation
        {
            Node *left = to_this->left_tree;
            Node *temp = left->right_tree;
            left->right_tree = to_this;
            to_this->left_tree = temp;
            to_this = left;
        ...
        {
            Node *b  = left_tree, *d  = b->right_tree, *DL = d->left_tree, *DR = d->right_tree;
            d->left_tree  = b;
