 ...
 *  Mutators
 *
 *   insert n b   	insert      the element can be pushed (always succeeds)
 *   erase n b    	erase      the element can be pushed (always succeeds)
 *   clear    		clear      the element can be pushed (always succeeds)
 *
 *  Others
 *
...
void Search_tree_tester<Type>::process() {
	if ( command == "new" ) {
		...
		Search_tree_iterator_tester<Type> tester( &begin_iterator );
		tester.run();
	} else if ( command == "end" ) {
		std::cout << "Okay" << std::endl;
		auto end_iterator = object->end();
		Search_tree_iterator_tester<Type> tester( &end_iterator );
		tester.run();
	} else if ( command == "rbegin" ) {
		std::cout << "Okay" << std::endl;
		auto rbegin_iterator = object->rbegin();
		Search_tree_iterator_tester<Type> tester( &rbegin_iterator );
		tester.run();
	} else if ( command == "rend" ) {
		std::cout << "Okay" << std::endl;
		auto rend_iterator = object->rend();
		Search_tree_iterator_tester<Type> tester( &rend_iterator );
		tester.run();
		...
		Search_tree_iterator_tester<Type> tester( &found_item_iterator );
		tester.run();
